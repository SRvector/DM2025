...
def extract_transactions(df):
    """
    Extract hashtags from clean_text into transactions.
    """
    tags = df['clean_text'].str.findall(r"#(\w+)")
    return tags.apply(lambda lst: [t.lower() for t in lst]).tolist()
...
def main():
    ...
    min_support = st.sidebar.slider("Min Support", 0.005, 0.2, 0.01, 0.005)
    ...
    # Compute Apriori frequent itemsets
    ...
    if freq_itemsets.empty:
        ...
        if rules.empty:
            st.warning("No rules found. Lower confidence or lift thresholds.")
        else:
    ...
    with tab3:
        ...
        samples = df[df["cluster"] == sel]["clean_text"].sample(min(5, len(df[df["cluster"]==sel])))
